...
- 自然：用户描述开发需求时自动匹配
- 额外：`/dwf:debug` 进入Debug流程，`/dwf:refactor` 进入重构评估，`/dwf:audit` 进入安全审计，`/dwf:retro` 进入周回顾
...
用户需求
  │
  ├─ 包含"debug/修复/为什么挂了/报错了"？
  │   └─ YES → 🔍 Debug模式
  │
  ├─ 已有项目 + 只改1-2个文件 + 不加新模块？
  │   └─ YES → 🏃 Quick模式（3步：改→测→交）
  │
  ├─ 新项目 或 需要新模块 或 改>10文件 或 影响架构？
  │   └─ YES → 🏗️ Full模式（11步+安全审计）
  │
  └─ 其他 → 📋 Standard模式（11步）
```

**需求清晰度判断**：
- **清晰**：用户明确说了做什么 + 范围 + 期望结果（如"给X加Y功能，输出Z"）
- **模糊**：只说了方向没说细节（如"优化一下"）→ 进入Step 2需求探索
- **矛盾**：说了互相冲突的需求 → 列出矛盾点让用户选择
...
### Step 0.3: 重构评估（已有项目，自动+手动）⭐ NEW

Step 0 自动触发快速重构扫描（可配置关闭），评估代码健康度：
...
评分输出：🟢 90-100 健康 | 🟡 70-89 可接受 | 🟠 50-69 需关注 | 🔴 0-49 技术债

手动深度评估：`/dwf:refactor` → 架构分析 + 循环依赖 + God Object 检测
重构原则参考：`references/refactor-principles.md`（6大原则+检查清单）
### Step 1: 接收需求
...
| Task | 50-200行 | MiniMax M2.7 | 功能级，免费模型 |
...
### Task 1: JWT Token 生成（50-200行，MiniMax M2.7）
...
### Task 2: 认证中间件（50-200行，MiniMax M2.7）
- Sub-task 2.1: ...
```
...
| Coder | MiniMax M2.7 Free | MiniMax M2.5 | GLM-5.1/Kimi K2.5 |
...
- 简单→MiniMax M2.5(免费) | 中等→MiniMax M2.7(免费) | 困难→GLM-5.1(付费)
