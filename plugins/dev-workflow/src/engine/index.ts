...
export class DevWorkflowEngine {
  ...
  async executeWorkflow(requirement: string): Promise<string> {
    ...
    // v6: UltraQuick 模式只跑 Step 3(需求) + Step 7(开发)

    if (this.context.mode === "ultra") {

      await this.runStep("step3-requirement", async () => {

        this.context!.decisions.push(`UltraQuick: ${requirement.slice(0, 100)}`);
      ...
      await this.runStep("step7-development", async () => {
        ...
        if (!this.context!.spec) {
          ...
          this.context!.spec = await this.orchestrator.defineSpec(requirement, this.context!.projectDir, []);
    ...
    try {
      ...
      if (this.context.mode !== "quick") {
        ...
        await this.runStep("step3-brainstorm", async () => {
      ...
      if (this.context.mode !== "quick" && this.context.featureFlags.githubIntegration) {
        ...
        await this.runStep("step12-delivery", async () => {
  ...
  /** state.json 持久化：读取 */
  private loadStateJson(projectDir: string): { step: number; phase: number; tasks: Record<string, { status: string }> } | null {
    const stateDir = join(projectDir, ".dev-workflow");
    const statePath = join(stateDir, "state.json");
    if (!existsSync(statePath)) return null;
    try {
      const data = JSON.parse(readFileSync(statePath, "utf-8"));
      if (this.context) {
        this.context.stateJsonPath = statePath;
        this.context.currentPhase = data.current_phase ?? 1;
      }
      return data;
    } catch { return null; }
  }
  ...
  /** state.json 持久化：保存 */
  private saveStateJson(): void {
    if (!this.context) return;
    const stateDir = join(this.context.projectDir, ".dev-workflow");
    const statePath = join(stateDir, "state.json");
    try {
      if (!existsSync(stateDir)) mkdirSync(stateDir, { recursive: true });
      const taskProgress: Record<string, { status: string; files: string[] }> = {};
      if (this.context.spec) {
        for (const t of this.context.spec.tasks) {
          if (t.status !== "pending") {
            taskProgress[t.id] = { status: t.status, files: t.files };
          }
        }
      }
      const state = {
        ...
        current_phase: this.context.currentPhase,
        current_step: this.context.currentStep,
        started_at: this.context.startedAt,
        updated_at: new Date().toISOString(),
        steps_completed: this.context.decisions.length,
        plan_gate_passed: this.context.currentPhase >= 3,
        task_progress: taskProgress,
        decisions: this.context.decisions,
        tech_stack: this.context.techStack,
      };
  ...
  /** state.json 持久化：归档 */
  private archiveStateJson(): void {
    if (!this.context?.stateJsonPath) return;
    try {
      const historyDir = join(this.context.projectDir, ".dev-workflow", "history");
      if (!existsSync(historyDir)) mkdirSync(historyDir, { recursive: true });
      const ts = new Date().toISOString().replace(/[:.]/g, "-");
      const dest = join(historyDir, `${ts}.json`);
      
      renameSync(this.context.stateJsonPath, dest);
    } catch { /* skip */ }
  }

  /** v6: Standard 模式轻量密钥扫描 */
  private async runLightweightSecurityScan(): Promise<string[]> {
    if (!this.context) return [];
    const patterns = ["API_KEY", "SECRET_KEY", "PASSWORD", "PRIVATE_KEY", "Bearer "];
    const findings: string[] = [];
    try {
      for (const pattern of patterns) {
        const stdout = execSync(`grep -rn "${pattern}" --include="*.ts" --include="*.js" --include="*.py" . | grep -v node_modules | grep -v ".example" | head -5`, { cwd: this.context.projectDir, stdio: "pipe", timeout: 10000 }).toString();
        if (stdout.trim()) {
          findings.push(...stdout.trim().split("\n").map((l: string) => `[SEC] ${l}`));
        }
      }
  ...
  private detectTechStack(projectDir: string): string[] {
    const stack: string[] = [];
    if (existsSync(join(projectDir, "package.json"))) stack.push("typescript");
    if (existsSync(join(projectDir, "tsconfig.json"))) stack.push("typescript");
    if (existsSync(join(projectDir, "requirements.txt")) || existsSync(join(projectDir, "pyproject.toml")) || existsSync(join(projectDir, "setup.py"))) stack.push("python");
    if (existsSync(join(projectDir, "pom.xml")) || existsSync(join(projectDir, "build.gradle"))) stack.push("java");
  ...
  /** v6: 按技术栈注入历史经验到 context */
  private async injectLessons(projectDir: string): Promise<void> {
    if (!this.context) return;
    const lessonsDir = join(__dirname, "..", "..", "skills", "dev-workflow", "references", "lessons");
    // 技术栈到经验文件的映射
    const stackToFile: Record<string, string> = {
    ...
    // 始终注入通用经验
    const alwaysFiles = ["testing.md", "git.md", "security.md"];
    const filesToInject = new Set<string>(alwaysFiles);
    for (const tech of this.context.techStack) {
      const file = stackToFile[tech];
      if (file) filesToInject.add(file);
    }
    let injectedCount = 0;
    for (const file of filesToInject) {
      ...
      if (existsSync(filePath)) {
        try {
          ...
          if (content) {
            this.context.decisions.push(`[经验注入] ${file}: ${content.slice(0, 200)}`);
            injectedCount++;
          }
        } catch { /* skip */ }
      }
    ...
    if (injectedCount > 0) {
      ...
      logger.info(`[dev-workflow] v6: 注入 ${injectedCount} 条历史经验（技术栈: ${this.context.techStack.join(", ")})`);
    }
  }
}
